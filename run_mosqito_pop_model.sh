...
# USAGE: ./run_mosquito pop_model.sh MOSQUITO_POP_MODEL_PATH CONFIG_PATH MOSQUITO_POP_CONFIG_FILENAME MOSQUITO_POP_OUTPUT_PATH MOSQUITO_POP_LOGS_PATH MINICONDA_PATH 
...
if [ "$#" -lt 7 ] || ! [ -d "$1" ] || ! [ -d "$2" ] || ! [ -f "$1/$3" ] || ! [ -d "$4" ] || ! [ -d "$5" ] || ! [ -d "$6" ] || ! [ -d "$7" ] ; then
